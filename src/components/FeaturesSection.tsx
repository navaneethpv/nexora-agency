            ...
            <div className="section-container">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                    {features.map((feature, index) => (
                        ...
                        >
                            <motion.div
                                whileHover={{ y: -8, borderColor: "rgba(11, 185, 243, 0.4)" }}
                                className="glass-card p-6 md:p-8 rounded-3xl md:rounded-4xl transition-all duration-500 group h-full relative"
                            >
                                <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-accent/10 flex items-center justify-center text-accent mb-5 md:mb-6 group-hover:bg-accent group-hover:text-white transition-all duration-500 shadow-[0_0_15px_rgba(11,185,243,0.1)]">
                                    <feature.icon className="w-5 h-5 md:w-6 md:h-6" />
                                </div>
                                <h3 className="text-lg md:text-xl font-bold mb-2 md:mb-3 group-hover:text-accent transition-colors duration-300">{feature.title}</h3>
                                ...
                                <div className="absolute inset-0 bg-accent/5 opacity-0 group-hover:opacity-100 blur-2xl transition-opacity duration-500 -z-10" />
                            </motion.div>
                        </ScrollReveal>
                    ))}
                </div>
